...
namespace Revit.SDK.Samples.UIAPI.CS.Starwood
{
    ...
    public class LoadedDie
    {
        ...
        // add up to 9.
        public LoadedDie(int probs, Random rr)
        {
            this.random = rr;
        ...
        public LoadedDie(IEnumerable<int> probs, Random rr)
        {
        ...
        public LoadedDie(IEnumerable<int> probs, Random rr, List<UnitType> typelist)
        {
            // Raise an error if nil
        ...
        public UnitType pickunitType(List<UnitType> typelist)
        {
            List<int> distlist = new List<int>();
            for (int i = 0; i < typelist.Count; i++)
            {
                if (typelist.ElementAt(i).unitdone == false)
                {
                    distlist.Add(typelist.ElementAt(i).numberofunits);
                }
                //else { sb.Append("done"); }
            }
            var loadedDie = new LoadedDie(distlist.ToArray(), random);

            //pick a type from that list
            int number = loadedDie.NextValue();
            UnitType ut = typelist.ElementAt(number);
            return ut;
    ...
    public class LoadedDie2
    {
        ...
        public LoadedDie2(Random rr, List<UnitType> typelist)
        {
            // Raise an error if nil
            List<int> distlist = new List<int>();
            for (int i = 0; i < typelist.Count; i++)
            ...
            IEnumerable<int> probs = distlist;
